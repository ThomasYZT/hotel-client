      ...
      <div class="flex-box">
        ...
        <div class="data-box right-box">
          ...
          <table-com v-if="showTable"
                     :data="tableData"
                     :config="refundCollectTableConfig"
            ...
            <template slot="col5"
              ...
              <el-table-column :prop="item.prop"
                ...
                <template slot-scope="{ row }">
                  <span>{{payTypeList.find(item => item.value === String(row.payType)) ?
                    payTypeList.find(item => item.value === String(row.payType)).label : ''}}</span>
                </template>
              </el-table-column>
            </template>
            <template slot="col6"
              ...
              <el-table-column :prop="item.prop"
                               ...
                               :min-width="item.minWidth">
                <template slot-scope="{ row }">
                  <span>{{statePayTypeList.find(item => item.value === Number(row.type)) ?
                    statePayTypeList.find(item => item.value === Number(row.type)).label : ''}}</span>
                </template>
              </el-table-column>
            </template>
            <template slot="col7"
              ...
              <el-table-column :prop="item.prop"
                               ...
                               :min-width="item.minWidth">
                <template slot-scope="{ row }">
                  <span>{{$util.toYuan(row.money) || 0}}</span>
                </template>
              </el-table-column>
            </template>
            <template slot="col8"
              ...
              <el-table-column :prop="item.prop"
                               ...
                               :min-width="item.minWidth">
                <template slot-scope="{ row }">
                  <span>{{refundStatusList.find(item => item.value === Number(row.status)) ?
                    refundStatusList.find(item => item.value === Number(row.status)).label : ''}}</span>
                </template>
              </el-table-column>
            </template>
          </table-com>
...
<script>
import { refundCollectTableConfig } from './tableConfig.js';
import { userType, payTypeList, statePayTypeList, refundStatusList } from '../../../assets/enums';

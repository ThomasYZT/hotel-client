<template>
  <div style="height: 100%">
    ...
    <table-com :data="tableData"
               ...
               :config="receiveRecordTableConfig"
               :getList="getList">
      <template slot="col2"
                slot-scope="{ item }">
      ...
      <template slot="col4"
        ...
        <el-table-column :prop="item.prop"
                         ...
                         :min-width="item.minWidth">
          <template slot-scope="{ row }">
            <span>{{payTypeList.find(item => item.value === row.payType) ?
              payTypeList.find(item => item.value === row.payType).label : ''}}</span>
          </template>
        </el-table-column>
      </template>
      <template slot="col6"
        ...
        <el-table-column :prop="item.prop"
                         ...
                         :min-width="item.minWidth">
          <template slot-scope="{ row }">
            <span>{{statePayStatusList.find(item => item.value === row.status) ?
              statePayStatusList.find(item => item.value === row.status).label : ''}}</span>
          </template>
        </el-table-column>
      </template>
    </table-com>
...
<script>
import { payTypeList, userType, statePayTypeList, statePayStatusList } from '../../../../assets/enums/index';
import { receiveRecordTableConfig } from './tableConfig';
